    ...
    *height = image->h;
    ...
    for (y = 0; y < *height; ++y)
    {
        for (x = 0; x < *width; ++x)
        {
            ...
            setPixel(x, y, makeColor(p[2], p[1], p[0]));
        }
    }
    return 1;
}
...
void drawLineTestPattern(int posX, int posY, float diameter)
{
    ...
    drawLine(posX, posY, posX, posY, LINE_COLOR_AXIS);
    ...
    drawCircle(posX, posY, diameter, CIRCLE_COLOR);
}
...
void drawLineTestPatternAlt()
{
    ...
    drawCircle(posX, posY, diameter * 0.1f, { 0u, 0u, 255u, 255u });
    drawCircle(posX, posY, diameter * 0.095f, { 0u, 0u, 255u, 255u });
}
...
Model loadPolygonalModel(const char *filename)
{
    std::vector<Vector> vertices{ };
    std::vector<Triangle> faces{ };
