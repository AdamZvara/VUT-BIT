...
ht_init(test_table);
ht_item_t *a;
if ((a = ht_search(test_table, "Ethereum")) != NULL)
  printf("%s : %.2f", a->key, a->value);
else
...
INSERT_TEST_DATA(test_table)
ht_item_t *a;
if ((a = ht_search(test_table, "Terra")) != NULL)
  printf("%s : %.2f", a->key, a->value);
else
  printf("Item not found");
ENDTEST
...
ht_init(test_table);
INSERT_TEST_DATA(test_table)
ht_delete(test_table, "Ethereum");
ht_delete(test_table, "Terra");
ht_delete(test_table, "Avalanche");
ht_delete(test_table, "Polkadot");
ENDTEST
